...
describe('DeliberationHandler — pvg-rotate rotation mode', () => {
  ...
  it('plants biased-prover role, stores debrief, sends keyboard, suppresses system-debrief broadcast', async () => {
    ...
    // Workers: agent-a gets biased-prover trailer, agent-b gets critic (no trailer)
    ...
    const handler = new DeliberationHandler(bus, [workerA, workerB], minConfig, sendFn, {
      sendKeyboardFn,
      pvgRotateStore,
    });
    ...
    // buildRotationKeyboard produces 4 buttons in 2 rows (2+2)
  ...
  it('does NOT activate rotation when pvgRotate is not set — existing adversarial path unchanged', async () => {
    ...
    const handler = new DeliberationHandler(
      bus,
    ...
    // No pvg-rotate session created
    expect(pvgRotateStore.get(threadId)).toBeUndefined();

    // sendKeyboardFn NOT called for rotation keyboard (might be called for blind-review but we didn't set blindReview)
    // system-debrief IS sent (because adversarialMode path runs)
    const systemDebriefCalls = sendFn.mock.calls.filter((c: unknown[]) => c[0] === 'system-debrief');
